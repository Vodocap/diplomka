...
impl WasmMLPipeline
{
    ...
    pub fn compute_matrix_r2(&self, indices_js: JsValue) -> Result<JsValue, JsValue>
    {
        ...
        for i in 0..k
        {
            ...
            for j in (i + 1)..k
            {
                let c = crate::entropy::mi_estimator::pearson_correlation(&columns[i], &columns[j]);
                corr_xx[i][j] = c;
                corr_xx[j][i] = c;
            }
        }
        ...
        let r_yx: Vec<f64> = columns.iter()
            .map(|col| crate::entropy::mi_estimator::pearson_correlation(col, y_data))
            .collect();
    ...
    fn compute_and_cache_matrices(&self, columns: &[Vec<f64>]) -> (Vec<Vec<f64>>, Vec<Vec<f64>>, Vec<Vec<f64>>)
    {
        ...
        let mi_matrix = crate::entropy::mi_estimator::compute_mi_matrix(columns, k_neighbors);
    ...
    #[wasm_bindgen(js_name = getFeatureMatrices)]
    pub fn get_feature_matrices(&self, data: &str, format: &str) -> Result<JsValue, JsValue>
    {
        if format != "csv"
        {
            return Err(JsValue::from_str("Maticová vizualizácia je zatiaľ podporovaná len pre CSV formát"));
        }

        let (columns, headers, num_rows, _skipped) = self.parse_csv_data_cached(data, true)?;

        if columns.len() < 2
        {
            return Err(JsValue::from_str("Potrebné sú aspoň 2 numerické stĺpce"));
        }
    ...
    {
        crate::entropy::mi_estimator::estimate_mi_ksg(x_col, y, k)
    }
            ...
            "with_selected" =>
            {
                for &u_idx in &unselected_indices
                {
                    for &s_idx in &selected_indices
                    {
                        ...
                        let joint_mi = crate::entropy::mi_estimator::estimate_joint_mi_ksg(
